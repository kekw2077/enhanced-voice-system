...
USE_GPU=0
# Сразу о неприятном: у AMD пути на видеокарту здесь НЕТ. Образ gigastt собран
# поверх sherpa-onnx, а его сборки с GPU существуют только под CUDA. Раньше
# скрипт просто не находил nvidia-smi и молча уходил на процессор — со стороны
# это выглядело как «видеокарта не подхватилась», хотя подхватывать нечего.
# Говорим прямо, тем более что процессору тут и правда хватает: замер на живых
# фразах — 141 мс на трёхсекундную фразу, при 600 мс ожидания тишины до неё.
if lspci -nn 2>/dev/null | grep -Ei 'vga|display|3d' | grep -q '\[1002:'; then
  warn "Видеокарта AMD: ускорения для этого движка не существует (sherpa-onnx"
  warn "собирается с GPU только под CUDA). Разворачиваю на процессоре — этого"
  warn "достаточно: 141 мс на фразу против 600 мс ожидания тишины перед ней."
fi
if [ "$FORCE" = "gpu" ]; then
  USE_GPU=1
elif [ "$FORCE" != "cpu" ]; then
