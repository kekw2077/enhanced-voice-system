...
const List<ChangelogEntry> kChangelog = [
  ChangelogEntry('2.14.0', [
    'Запуск больше не переустанавливает движок, который уже установлен. Скачанное обновление после распаковки не всегда удавалось удалить — Windows ещё держала файл, — и на каждом следующем запуске приложение заново считало контрольную сумму по 112 МБ и распаковывало 275 МБ поверх того же самого. На машине с таким файлом это 88 секунд из 94: теперь версия проверяется до всей работы, а не после.',
    'Список компонентов читается с диска, а сеть опрашивается в фоне. Чтобы решить, какой файл движка запускать, свежий список не нужен — нужен тот, по которому движок уже установлен. Раньше запрос стоял на пути запуска с таймаутом 15 секунд.',
    'Модель распознавания берётся из локального кэша. Whisper на каждом запуске ходил на huggingface.co спросить, не вышла ли новая версия модели, которая и так лежит рядом: замер — 1,67 с против 0,20 с. В сеть приложение теперь идёт, только если модели действительно нет.',
    'Модель перестала загружаться дважды. Служба стартовала со своим размером модели по умолчанию, получала настоящий уже на ходу и грузила всё заново.',
    'Пока идёт загрузка, окно программы и виджет открыть нельзя — ни из трея, ни горячей клавишей, ни повторным запуском. Раньше окно можно было вытащить поверх окна загрузки и получить наполовину живое приложение. В трее на это время вместо «Показать EVS» — неактивное «Загрузка…».',
    'Виджет и окно появляются вместе, ровно в момент готовности принимать команды.',
    'Полоса загрузки перестала замирать: границы этапов разнесены по их настоящей длительности, добавлен отдельный этап «Прогрев движка», а внутри этапа полоса продолжает двигаться, пока работа идёт.',
  ]),
  ChangelogEntry('2.13.0', [
    'Появилось распознавание на сервере — третий движок рядом с Whisper и GigaAM. Фраза уходит на ваш сервер, распознаёт его видеокарта. Микрофон, шумоподавление и нарезка фраз остаются на этой машине: наружу уезжает готовая фраза, а не поток звука.',
    'Контракт взят чужой намеренно — OpenAI-совместимый POST /v1/audio/transcriptions. Такой эндпоинт из коробки отдают готовые серверы (speaches, whisper.cpp server, vLLM), то есть держать и обновлять собственную серверную часть не придётся.',
    'Если сервер не отвечает, фраза не пропадает: её подхватывает локальный Whisper. Три сорвавшихся запроса подряд переводят на локальный движок совсем — и об этом сообщается, а не происходит молча.',
    'С серверным движком приложение готово слушать заметно быстрее: локальную модель грузить не нужно, готовность наступает за полсекунды вместо трёх.',
    'В «Общих» появился выбор, что показывать после запуска: только виджет, окно и виджет, только окно или ничего. Раньше это решал тумблер «Плавающий виджет», который заодно прятал окно, и «окно вместе с виджетом» выразить было нечем.',
    'Строки состояния перестали называть Whisper, когда работает другой движок: и чип в настройках, и телеметрия в Nexus и «Ноктюрне» теперь берут название из одного места.',
  ]),
  ChangelogEntry('2.12.0', [
    'Push-to-Talk наконец работает. Раньше переключатель «по нажатию» был мёртвым: значение сохранялось, но его не читала ни одна строка кода. Теперь под ним появляется область назначения — жмёте «Назначить» и зажимаете сочетание из одной, двух или трёх клавиш.',
    'Удержание клавиши заменяет слово-активатор: сказанное при зажатой комбинации сразу считается командой, звать «Айрис» не нужно. «Режим распознавания» в этом режиме ни на что не влияет, и об этом теперь написано прямо под настройкой.',
    'В режиме удержания микрофон открыт только пока клавиша зажата — постоянной записи нет. После отпускания запись длится ещё мгновение: сайдкар закрывает фразу по паузе, и мгновенный обрыв съедал бы последнее слово.',
    'EVS не перехватывает назначенную клавишу — она продолжает работать в других программах. Если удержание совпадает с сочетанием показа окна, настройки об этом предупреждают.',
    'Экраны перестали говорить «Слушаю», когда микрофон закрыт. При Push-to-Talk состояние покоя подсказывает, что зажать, подпись у поля ввода и капсула в шапке показывают комбинацию вместо слова-активатора, а в настройках вместо «Микрофон: слушает» — «по удержанию клавиши».',
    'Запуск приложения показывает окно загрузки. Вместо заставки, которая крутилась ровно 4,8 секунды по таймеру и никак не была связана с готовностью (а при включённом виджете её вообще было не видно), теперь отдельное окно с этапами: настройки, трей, скачивание компонентов с процентами, запуск службы распознавания, загрузка моделей. Окно висит до тех пор, пока ассистент действительно не готов слушать, а проверка обновлений идёт отдельной строкой и запуск не задерживает.',
    'Пока висит окно загрузки, главное окно скрыто — раньше при выключенном виджете оба появлялись одновременно.',
    'Журнал перестал терять строки. Записи шли в файл без очереди, и при одновременных событиях строки перетирали друг другу хвост — в журнале оставались обрубки.',
  ]),
  ChangelogEntry('2.11.0', [
    'Чат в «Ноктюрне» теперь действительно отключается. Тумблер был, но поле ввода оставалось живым: блокировка была написана внутри композеров классического стиля и Nexus, а в «Ноктюрне» её просто не было. Теперь вместо поля — замок с той же подписью, что и в остальных стилях, а сам запрет стоит в ядре, чтобы следующий стиль не смог повторить ту же ошибку.',
    'Поиск в интернете научился работать через Google и Яндекс. К прежним Tavily, Brave и DuckDuckGo добавились Google Programmable Search (нужны ключ и ID движка) и Yandex Search API (нужны ключ и folderid). Ключи вводятся там же, в «Веб-поиске».',
    'Появился явный выбор поисковика. Раньше брался первый, у кого задан ключ, и порядок был жёстко зашит. Теперь есть «Авто» (прежнее поведение) и возможность указать конкретный. Заодно в режиме «Авто» пустой ответ одного поисковика больше не означает конец поиска — спрашивается следующий.',
    'Сервер клон-голоса запускается кнопкой. Раньше qwen_tts_server.py поднимали руками из консоли; теперь он стартует и останавливается из «Голос ассистента → Движок синтеза». Перед остановкой видеопамять освобождается штатно, а сам процесс привязан к приложению — осиротеть и держать карту он не может. Интерпретатор с torch ищется сам, путь можно указать вручную.',
  ]),
  ChangelogEntry('2.10.3', [
    'Новая иконка приложения и новый значок в трее. Ярлык, окно установщика и сам exe получили детальную прорисовку знака, трей — упрощённую: она нарисована под мелкие размеры, полосы в ней шире и разнесены дальше.',
    'Значок в трее наконец читается. В кадре 16 пикселей (обычный масштаб экрана) зазор между полосами получается меньше пикселя, и при обычном уменьшении четыре полосы сливались в один смазанный столбик. Теперь кадры 16, 20, 24 и 32 выравниваются по пиксельной сетке — полосы видно раздельно.',
    'Знак внутри приложения не менялся: на заставке, в углу главного экрана и в «О программе» остаётся Genesis. Системные иконки и знак в приложении различаются намеренно.',
    'Убрана ошибка, из-за которой при открытии главного экрана «Ноктюрна» строка состояния могла пропустить первое обновление.',
  ]),
  ChangelogEntry('2.10.2', [
    'Кольцо на главном экране «Ноктюрна» вращается само, а не после нажатия. Его останавливала настройка «Движение интерфейса»: в режиме «Сбалансированная» анимации идут только пока ассистент занят, а нажатие на кольцо как раз запускало ассистента. Теперь у кольца и знака EVS свой режим — они идут, пока окно на экране; «Экономная» и свёрнутое окно их по-прежнему останавливают.',
    'Знак EVS в «О приложении» снова живой и не замирает.',
    'Кнопка микрофона в «Ноктюрне» заменена на включение/отключение чата — ту же настройку, что в «Голосовые команды → Разрешить чат». Микрофон слушает сам, и кнопка «нажми, чтобы сказать» рядом с полем ввода ничего не давала; в классическом стиле эту замену сделали раньше.',
    'Вкладка «Модели» в «Ноктюрне» открывает настоящий выбор модели (Нейросеть → Подключение) — те модели, что подтягиваются с подключённого сервера. Свой каталог моделей «на устройстве» убран: десктопная версия такой режим не поддерживает, и вкладка предлагала скачать то, чем приложение не смогло бы воспользоваться.',
    'Кнопка «Журнал» в стиле Nexus открывает журнал работы приложения, а не список изменений. Появилась страница «Журнал» в настройках — поиск, фильтры по источникам, счётчик ошибок, кнопка «Папка логов»; это те же записи, что показывает «Ноктюрн». История изменений осталась на своём месте, в «О приложении».',
  ]),
  ChangelogEntry('2.10.1', [
    'Иконка приложения и значок в трее — тот же знак Genesis, что и внутри приложения. Раньше в панели задач и трее оставался прежний рисунок, и одно с другим не сходилось.',
    'Для трея знак взят крупнее общего плана: в кадре 16 пикселей кольцо на общем плане просто теряется.',
  ]),
  ChangelogEntry('2.10.0', [
    'Третий стиль интерфейса — «Ноктюрн» (Настройки → Внешний вид → Стиль интерфейса). Если «Классический» — рабочий стол, а Nexus — пульт, то «Ноктюрн» — инструмент: вкладки сверху, сцена во всё окно, вся телеметрия одной строкой по низу. Состояние ассистента показывает цвет: ожидание — серый, слушаю — янтарный, думаю — сиреневый, говорю — голубой, и этот цвет красит только то, что относится к состоянию.',
    'В «Ноктюрне» четыре экрана вместо одного: «Диалог» — история отдельным экраном с поиском и группами вместо постоянной колонки; «Команды» — таблица с фразами, действиями и типами, опасные помечены, порог совпадения и подтверждение закреплены внизу; «Модели» — подключение сверху, каталог карточками; «Журнал» — моноширинный поток по реальным логам с фильтрами и кнопкой «Папка логов». Главный экран открывается по марке EVS слева.',
    'Новый логотип — Genesis. При запуске он собирается на весь экран: вспышка из сингулярности, раскручивается кольцо, горизонт событий «съедает» свет, прорисовывается доплеровский блик, прилетают орбиты и свуши, глитч прошивает круг — и появляется подпись «Enhanced Voice System». Нарисован в приложении целиком, кадр за кадром по оригинальной анимации, а не картинкой.',
    'Тот же знак заменил значок в левом верхнем углу главного экрана и в «О программе» — он живой: дуги вращаются, частицы по спирали падают на горизонт событий, кольцо дышит. В режимах экономии движения («Настройки → Внешний вид → Движение») знак замирает, как и остальные анимации.',
    'Пока ставится обновление, экран больше не пустой. Приложение в этот момент закрыто (иначе установщик не заменит файлы), поэтому логотип показывает отдельное окно с подписью «Установка обновления» и полосой прогресса — она отражает реальный ход дела (скачивание, ожидание закрытия, установка, перезапуск), а не бежит для вида. Окно закрывается само, когда новая версия запустилась.',
    'Мелочь по пути: с включённым плавающим виджетом окно приложения стартует скрытым — теперь оно в этом состоянии не тратит кадры на анимации, которых никто не видит, а заставка ждёт первого показа окна.',
  ]),
  ChangelogEntry('2.9.3', [
    'Исправлено: кнопка «Включить клон-голос» и выбор клон-движка ничего не делали, пока клон-сервер выключен. Разблокировать кнопки было мало — запрет сидел глубже, в самой настройке, и она молча отклоняла выбор. Именно из-за этого заготовленные фразы оставались недоступны: кэш читается только при выбранном клоне, а выбрать его было нельзя.',
    'Теперь клон-голос выбирается при выключенном сервере — нужен только образец и адрес. Заготовки играют вашим голосом, всё остальное озвучивает Piper (или системный голос, если Piper не установлен).',
  ]),
  ChangelogEntry('2.9.2', [
    'Кнопка прослушивания в «Заготовленных фразах» теперь играет саму заготовку — запись вашего голоса, — а не проговаривает эту фразу текущим движком. Раньше при выбранном Piper она отвечала голосом Piper, и это выглядело так, будто кэш не работает.',
    'В той же карточке видно, когда заготовки не используются: если выбран обычный движок озвучки, появляется предупреждение и кнопка «Включить клон-голос». Прослушивание при этом всё равно играет запись клона.',
    'Команды с телефона наконец выполняются. Раньше фраза с телефона уходила сразу в нейросеть в обход каталога команд: «открой браузер» вызывало ответ модели вместо запуска браузера, а при недоступном сервере модели — ошибку на команду, которой модель вообще не нужна. Теперь телефон проходит тот же путь, что и речь на компьютере: сначала каталог команд, чат — только если ничего не совпало.',
    'Ответ на телефон больше не проговаривается дважды, когда команда уже сообщила о себе вслух.',
  ]),
  ChangelogEntry('2.9.1', [
    'Исправлено: заготовленные фразы клон-голоса не воспроизводились — говорил системный голос. Причин было две. Первая: движок клона нельзя было выбрать, пока сервер выключен (кнопка блокировалась), а кэш заготовок читается только при выбранном клоне — выхода из этого состояния в интерфейсе не было. Вторая: сайдкар искал кэш только у активного движка, поэтому остановленный сервер прятал уже готовые фразы за системным голосом. Теперь кэш проверяется первым и не зависит от того, какой движок запущен сейчас.',
    'Клон-сервер восстанавливается сам: как только он снова доступен, EVS возвращается на него в течение минуты — заходить в настройки не нужно.',
    'Разовая починка настроек: если клон настроен, а голос Piper не установлен, движок озвучки возвращается на клон (прошлые версии молча переключали его на Piper, когда сервер не отвечал).',
    'В строке состояния вместо «не отвечает» — «сервер офлайн, заготовки из кэша»: видно, что голос всё ещё ваш.',
  ]),
  ChangelogEntry('2.9.0', [
    'Настройки стали двухуровневыми: у раздела в левой панели раскрываются подразделы, и каждый открывает короткую страницу вместо длинной прокрутки. Например «Голос ассистента» теперь разложен на «Озвучку», «Движок синтеза», «Клон и нагрузку GPU», «Эффекты» и «Заготовленные фразы».',
    'Разобрана самая длинная карточка настроек: выбор движка, блок клона, ползунки видеокарты, интерпретатор и эффекты голоса лежали в одной карточке, из-за чего средняя колонка тянулась на несколько экранов вниз. Теперь это отдельные страницы, а колонки выравниваются по высоте.',
    'Поиск по настройкам ведёт прямо на нужную страницу и показывает путь до неё («Голос ассистента · Клон и нагрузка GPU»). Ищутся и названия подразделов.',
    'Исправлены переходы из главного экрана: карточки «LLM» и «Система» открывали раздел «Телефоны» вместо настроек нейросети и видеокарты, а карточка «TTS» вела в микрофон. Переходы больше не привязаны к порядку разделов, поэтому не разъедутся снова.',
    '«История изменений» — отдельная страница и показывает 12 последних версий вместо трёх.',
  ]),
  ChangelogEntry('2.8.1', [
    'Исправлено: привязанный телефон пропадал после перезапуска. Привязка происходит внутри окна настроек, где сохранение отложено до нажатия «Сохранить», поэтому устройство не попадало на диск, а при выходе из настроек стиралось и из памяти. Теперь привязка сохраняется сразу.',
    'Исправлено: заготовленные фразы клон-голоса не звучали — вместо них включался системный голос. Когда клон-сервер не отвечал, приложение само переключало озвучку на обычный движок, а кэш фраз используется только при активном клон-движке. Теперь клон остаётся выбранным: заготовленные фразы играют вашим голосом даже с выключённым сервером, а новый текст озвучивается запасным голосом.',
    'Новая карточка «Заготовленные фразы» (Настройки → Голос ассистента): видно весь список заготовок, каждую можно прослушать, и есть кнопка заготовить заново.',
  ]),
  ChangelogEntry('2.8.0', [
    'Настройки перестроены: перегруженный раздел «Голосовой ввод» разделён на два — «Микрофон и распознавание» (устройство, усиление, чувствительность, шумоподавление, язык) и «Голос ассистента» (движок озвучки, голоса, скорость и громкость, эффекты, клон и нагрузка на видеокарту). Уточнены названия разделов: «Модель и инференс» → «Нейросеть», «Виджеты» → «Виджеты и оверлей».',
    'Кнопка включения и выключения чата — на месте бывшей кнопки голосового ввода. Выключенный чат означает режим «только голосовые команды».',
    'Чат можно включать и выключать голосом: создайте команду (Команды → Системное → «Включить чат» / «Выключить чат») и скажите, например, «Айрис, включи чат» — после этого можно сразу задать вопрос. Чат выключается только явно: командой или кнопкой.',
  ]),
  ChangelogEntry('2.7.4', [
    'Поиск по настройкам — строка вверху окна настроек. Ищет и по названиям, и по описаниям: наберите «громкость», «тихо» или «не слышно» — и нужная настройка найдётся вместе с разделом, в котором лежит. Клик по результату открывает раздел.',
    'Строка состояния в разделе «Голос»: сразу видно, что реально работает прямо сейчас — микрофон, движок распознавания и каким голосом отвечает ассистент. Если голос не выбран и говорит системный, или клон-сервер не отвечает — это теперь видно, а не выясняется на слух.',
    'Из чата убрана кнопка голосового ввода — микрофон и так слушает постоянно.',
  ]),
  ChangelogEntry('2.7.3', [
    'Обновление больше не показывает чёрное окно командной строки: служебный сценарий, который ждёт закрытия приложения и ставит новую версию, теперь выполняется полностью скрыто. Сам процесс обновления не изменился.',
  ]),
  ChangelogEntry('2.7.2', [
    'Установщик: при первой установке можно выбрать, что доставить сразу — голосовой движок, распознавание речи GigaAM-v3, шумоподавление. Выбранное скачивается прямо во время установки, и приложение стартует уже готовым. Ничего не выбрали — всё по-прежнему можно скачать из самого приложения. Установщик получил фирменное оформление.',
  ]),
  ChangelogEntry('2.7.1', [
    'Стиль Nexus — окно настроек приведено к общему оформлению: слева остаётся рейл с иконками (настройки больше не «отдельный экран»), появился заголовок «Настройки» с подсказкой закрытия, а сам Esc теперь закрывает окно. Иконки рейла переключают раздел прямо в настройках. Цвета следуют выбранной теме. Классический стиль не изменился.',
  ]),
  ChangelogEntry('2.7.0', [
    'Усиление микрофона — новый ползунок (Настройки → Голос → STT): делает сигнал микрофона громче в 0.5–4 раза. Нужен, если вас плохо слышно или вы сидите далеко: слишком тихая речь не проходит порог громкости, каким бы чувствительным ни был детектор речи. Это отдельная настройка от «Чувствительности» — та решает, насколько строго считать звук речью. По умолчанию 1.0x, то есть без изменений.',
    'Свои голоса Piper: в разделе моделей появилась кнопка «Добавить свой голос» — можно подключить голос из архива .tar.bz2 или файла .onnx (недостающие данные произношения EVS доготовит сам). Раньше голос, положенный в папку вручную, просто не появлялся в списке.',
    'Обновлён голосовой движок (v18) — поддержка усиления микрофона. Применится автоматически при следующем запуске.',
  ]),
  ChangelogEntry('2.6.2', [
    'Запасной голос теперь Piper, а не системный «робот»: если клон-голос не смог озвучить фразу (например, во время игры, когда клон намеренно стоит в стороне), её произнесёт обычный нейросетевой голос. Системный голос Windows остался только на самый крайний случай — если голос Piper не установлен. Голосовой движок обновится автоматически при запуске.',
  ]),
  ChangelogEntry('2.6.1', [
    'Стиль Nexus — завершающая полировка: ползунки настроек и подсветка активного поля ввода теперь в фирменном акценте Nexus. Классический стиль не изменился.',
  ]),
  ChangelogEntry('2.6.0', [
    'Клон голоса на видеокарте (Qwen3-TTS) — замена вырезанному CPU-клону. Сервер запускается в вашем venv (инструкция: sidecar/GPU_CLONE_SETUP.md), EVS подключается к нему по адресу; движок в настройках называется «Клон (сервер)». Веса модели скачиваются на тот же диск, где стоит EVS. Тот же движок работает и с CosyVoice — достаточно указать его адрес, когда рендер переедет на сервер с видеокартой.',
    'Управление нагрузкой на видеокарту: профили Максимум / Баланс / Тихий (лимит видеопамяти, выгрузка из памяти в простое, сдерживание) плюс тонкая настройка в «Дополнительно».',
    'Клон и игры: по умолчанию во время игры знакомые фразы играют из кэша (видеокарта не трогается), новые — обычным голосом; можно выбрать «Как обычно» или «Выключать». После выхода из игры клон-голос возвращается сам.',
    'Исправлено: клон-сервер вообще не подключался как движок озвучки (настройки сохранялись, но не применялись), и не переживал переподключение голосового движка.',
    'Все файлы программы пишутся только на диск, где установлен EVS: временные файлы больше не создаются в системной папке Windows.',
  ]),
  ChangelogEntry('2.5.0', [
    'Клонировщик голоса на CPU (XTTS) полностью удалён из EVS: он давал невнятную озвучку, а его компонент вместе с кэшами занимал около 8 ГБ на диске. При обновлении EVS сам освобождает это место (удаляет остатки клона), и голосовой движок больше никогда не докачивает клон-компонент.',
  ]),
  ChangelogEntry('2.4.9', [
    'Клон голоса на CPU (XTTS) убран: озвучка получалась невнятной («набор звуков»). Голос возвращается к обычному Piper. GPU-рендер клонирования вернём позже отдельным решением.',
    'Регулировка громкости приложений переделана: команда теперь выставляет указанную вами громкость (значение задаётся при создании команды) и больше не зависит от распознавания числа в речи. Ушла ошибка «Не расслышал число» — и при голосовой команде, и при ручном запуске.',
    'Эффекты голоса (FX) снова применяются после полного перезапуска приложения — раньше голос сбрасывался на обычный, пока не откроешь настройки.',
    'Стиль Nexus: чат теперь блокируется как в классическом — ввод недоступен в режиме «только команды» и на время генерации ответа (нельзя отправить второе сообщение, пока не готов первый ответ).',
  ]),
  ChangelogEntry('2.4.8', [
    'Стиль Nexus — доведён скин настроек: активный раздел, иконки карточек, переключатели и радио-опции (движок STT, режим подключения, голоса) получили фирменный акцент Nexus. Классический стиль не изменился.',
  ]),
  ChangelogEntry('2.4.7', [
    'Подключение телефона: QR теперь по умолчанию отдаёт адрес локальной сети (Wi-Fi), а не Tailscale — телефон в той же сети подключался к недостижимому 100.x и «сканировал, но не соединялся». Добавлен выбор адреса (Wi-Fi / Tailscale) и видимый host:port для ручного ввода.',
    'Клон голоса: заготовки фраз теперь реально сохраняются в кэш (была ошибка записи из-за временного файла .part) — прогон один раз, дальше мгновенно, без повторной подготовки после команд. Обновите голосовой движок автоматически при запуске.',
  ]),
  ChangelogEntry('2.4.6', [
    'Исправлено «настройки не сохраняются»: часть настроек (эффекты голоса, анимации интерфейса, чувствительность микрофона, тумблер и образец клона, быстрые профили) не попадала в отслеживание изменений — правки тихо откатывались при выходе из настроек без вопроса о сохранении. Теперь все они корректно помечают изменения и сохраняются.',
    'Nexus: статус в левом верхнем углу реагирует на слово-активатор — при распознавании вспыхивает «(слово) — услышал, говорите!», затем «Говорите команду…» на время окна захвата.',
    'Обновления компонентов: подготовленный архив больше не удаляется при неудачной распаковке — повторная попытка на следующем запуске вместо повторной закачки.',
  ]),
  ChangelogEntry('2.4.5', [
    'Исправлена главная причина «клон голоса не работает»: голосовой движок (сайдкар) не мог обновляться после переезда в архив — установка навсегда оставалась на старой версии без поддержки клона, хотя сам клон-компонент был скачан. Теперь обновления движка качаются в фоне и применяются при следующем запуске.',
    'Диагностика клона: ошибки клон-воркера больше не теряются — падения и переключения движков пишутся в logs/sidecar.log (раньше сбой выглядел как «молча говорит старым голосом»).',
    'Обновлён голосовой движок (v14): поддержка клона + видимые логи. Применится автоматически при первом запуске после обновления.',
  ]),
  ...
  ChangelogEntry('2.4.3', [
    ...
    'Исправлено: выбранный образец WAV теперь сразу отображается в настройках.',
  ]),
  ChangelogEntry('2.4.2', [
    'Запуск от администратора без окна UAC: в мастере команд (для программ, файлов и shell-команд) появился переключатель «Запускать от администратора». При сохранении EVS один раз попросит подтверждение и создаст задачу Планировщика — дальше «запусти валорант» стартует без вопросов. UAC остаётся включён для всего остального.',
    'Интерпретатор озвучки: модель теперь выбирается из списка доступных на сервере, а «По умолчанию» следует за глобальной моделью чата. Исправлена подпись режима (показывался служебный ключ).',
  ]),
  ChangelogEntry('2.4.1', [
    'Оптимизация нагрузки на процессор: декоративные анимации (сфера, рамки, идл-волны) больше не крутятся впустую — в новом режиме «Умные» (по умолчанию) они работают только когда ассистент активен, и полностью замирают в трее. Регулировка — «Внешний вид» → «Анимации интерфейса» (Всегда / Умные / Экономия).',
    'Сфера теперь реагирует на звук во всех местах (чат, оверлей, виджеты) — раньше пульсировала только на голосовом экране.',
    'Настройки клона голоса (XTTS) и CosyVoice разделены на отдельные секции с явными заголовками — два образца голоса больше не путаются.',
    'Команды громкости с {N} исправлены: «громкость на 30» теперь распознаётся (число цифрами или словами), без числа применяется значение по умолчанию из настроек команды. В мастере — подсказка про {N}.',
    'Быстрые профили: активный подсвечивается, каждый можно настроить (что именно он меняет), есть сброс к стандартным.',
    'Исправлена верстка настроек (белая полоса над боковой панелью) и фон тёмной темы Claude (был холодный вместо тёплого).',
  ]),
...
// skin (rail + stage + chat) from the Nexus TZ; `noctur` = «Ноктюрн» — the same
// data in an instrument-like window (tabs on top, stage across the whole window,
// one monospace telemetry line at the bottom). A downgraded build that reads an
// unknown value falls back to `standard` via `orElse` in the prefs loader, so the
// field stays forward/backward compatible. (Liquid Glass, an old value, was
// removed.)
enum AppStyle { standard, nexus, noctur }
